...
CVspread::CVspread(fhicl::ParameterSet const &params)
    : IGENIESystProvider_tool(params) { //get values from associated fcl file
      ...
      dial_infos.emplace_back(DialInfo{"CVspread"});
      CVs.push_back(0);
...
SystMetaData BuildSystMetaData(fhicl::ParameterSet const &ps,
                                            paramId_t firstId) {
  SystMetaData smd;
  ...
  for (auto const &prettyname : dial_infos) {
    SystParamHeader phdr = dial_variation_template; //short-hand for simplicity
...
event_unit_response_t CVspread::GetEventResponse(genie::EventRecord const &ev){
  ...
  ISLepP4 = *ISLep->P4(); //Points to initial state four-vector
