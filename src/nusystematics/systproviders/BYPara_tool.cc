...
BYPara::BYPara(fhicl::ParameterSet const &params)
  ...
  for (int rescode = 0; rescode < 18; ++rescode) {
    ...
    for (auto const &[dial_name, gdial] :
         std::vector<std::pair<std::string, genie::rew::GSyst_t>>{
             {"AhtBY", genie::rew::kXSecTwkDial_AhtBY},
             {"BhtBY", genie::rew::kXSecTwkDial_BhtBY}}) {
...
SystMetaData BYPara::BuildSystMetaData(fhicl::ParameterSet const &ps,
                                            paramId_t firstId) {
  SystMetaData smd;
...
bool BYPara::SetupResponseCalculator(
...
BYPara::GetEventResponse(genie::EventRecord const &ev) {
